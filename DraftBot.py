...
class MyClient(discord.Client):
    ...
    async def on_raw_reaction_add(self,react):
        ...
        if str(react.emoji.id) == "1440771340213944490":
            channel = self.get_channel(react.channel_id)

            if channel is None:
                channel = await self.fetch_channel(react.channel_id)

            await channel.send("East Gastrodon is best Gastrodon!")
    ...
    async def on_message(self,message):
        ...
        if lookUp:
            ...
            if pokemonName == "obama":
                flag = await message.channel.send("Try looking for the goat instead.")
                await asyncio.sleep(1)
                await flag.delete()
            
            if pokemonName == "femboy":
                flag = await message.channel.send("Man I wish we could draft those...")
                await asyncio.sleep(1)
                await flag.delete()
